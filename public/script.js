...
// Verifica se há credenciais salvas ao carregar a página
...
function showLogin() {
  hideLoading(); // Garante que o loading está oculto
...
async function showApp() {
  ...
  // Mostra loading e aguarda carregar ambos os dados
  showLoading();
  try {
...
function logout() {
  if (
    confirm("Deseja realmente sair? Suas credenciais salvas serão removidas.")
  ) {
...
document.getElementById("login-form").addEventListener("submit", async (e) => {
  ...
  if (!apiKey || !apiSecret) {
    alert("Por favor, preencha ambas as credenciais");
    return;
  }
  ...
  try {
    ...
    // showApp já gerencia o loading internamente
    await showApp();
  } catch (error) {
    hideLoading();
    alert("Erro ao validar credenciais: " + error.message);
    API_KEY = null;
    API_SECRET = null;
  }
});
...
async function loadBalances() {
  ...
  try {
    ...
    const [data, raw, avgPrices] = await Promise.all([
      fetch("/balances", { headers }).then((x) => x.json()),
      fetch("/raw", { headers })
        .then((x) => x.json())
        .catch(() => null),
      fetch("/avg-buy-prices", { headers })
        .then((x) => x.json())
        .catch(() => ({})),
    ]);
    ...
    usdtUsdtDiv.innerText = `${
      data.assets.find((a) => a.asset === "USDT")?.totalUSDT.toFixed(2) || 0
    } USDT`;
    btcUsdtDiv.innerText = `${
      data.assets.find((a) => a.asset === "BTC")?.totalUSDT.toFixed(2) || 0
    } USDT`;
    xrpUsdtDiv.innerText = `${
      data.assets.find((a) => a.asset === "XRP")?.totalUSDT.toFixed(2) || 0
    } USDT`;
    ethUsdtDiv.innerText = `${
      data.assets.find((a) => a.asset === "ETH")?.totalUSDT.toFixed(2) || 0
    } USDT`;
...
async function renderAssetSummary(assets, quantityMap, avgPriceMap = {}) {
  ...
  const rows = await Promise.all(
    assets.map(async (asset) => {
      ...
      const valorizacao =
        cost !== null && currentValue !== null ? cost - currentValue : null;
...
async function loadOrders() {
  ...
  try {
    ...
    const json = await fetch("/orders", { headers }).then((x) => x.json());
...
function renderOrders() {
  ...
  totalOrdersDiv.innerHTML =
    "Total das ordens abertas filtradas: <b>" + total.toFixed(2) + " USDT</b>";
